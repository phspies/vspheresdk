...
{
    public class SettingsDepotContentBaseImagesVersionsModule
    {
        ...
        public SettingsDepotContentBaseImagesVersionsModule(RestClient Client, CancellationToken _cancellationToken, int _timeout, int _retry)
        {
            restClient = Client;
            retry = _retry;
            timeout = _timeout;
        ...
        public async Task<EsxSettingsDepotContentBaseImagesVersionsInfoType> GetAsync(string Version)
        {
            ArgumentNullException.ThrowIfNull(Version, "Version cannot be null");
            StringBuilder GetServiceURL = new StringBuilder("/api/esx/settings/depot-content/base-images/versions/{version}");
            ...
            RestResponse<EsxSettingsDepotContentBaseImagesVersionsInfoType> response = await restClient.ExecuteTaskAsyncWithPolicy<EsxSettingsDepotContentBaseImagesVersionsInfoType>(request, cancellationToken, timeout, retry);
            ...
            else if ((int)response.StatusCode == 404) { throw new vSphereException("if baseimage with given version is not found.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
