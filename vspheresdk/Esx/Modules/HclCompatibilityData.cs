...
{
    public class HclCompatibilityDataModule
    {
        ...
        public HclCompatibilityDataModule(RestClient Client, CancellationToken _cancellationToken, int _timeout, int _retry)
        {
            restClient = Client;
            retry = _retry;
            timeout = _timeout;
        ...
        public async Task<EsxHclCompatibilityDataStatusType> GetAsync()
        {
            StringBuilder GetServiceURL = new StringBuilder("/api/esx/hcl/compatibility-data/status");
            ...
            RestResponse<EsxHclCompatibilityDataStatusType> response = await restClient.ExecuteTaskAsyncWithPolicy<EsxHclCompatibilityDataStatusType>(request, cancellationToken, timeout, retry);
        ...
        {
            StringBuilder UpdateTaskServiceURL = new StringBuilder("/api/esx/hcl/compatibility-data?action=download&vmw-task=true");
            var request = new RestRequest
            {              
                RequestFormat = DataFormat.Json,
                Method = Method.Post
            ...
            if (200 <= (int)response.StatusCode && (int)response.StatusCode <= 300) { return response.Data; }
            else if ((int)response.StatusCode == 400) { throw new vSphereException("if there is compatibility data update in progress.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else if ((int)response.StatusCode == 401) { throw new vSphereException("if the caller is not authenticated.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else if ((int)response.StatusCode == 500) { throw new vSphereException("If there is some unknown error. The accompanying error message will give more details about the failure.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else { throw new vSphereException(response.ErrorMessage, (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); } 
            return response.Data;
