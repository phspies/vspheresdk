...
{
    public class SettingsDepotContentAddOnsVersionsModule
    {
        ...
        public SettingsDepotContentAddOnsVersionsModule(RestClient Client, CancellationToken _cancellationToken, int _timeout, int _retry)
        {
            restClient = Client;
            retry = _retry;
            timeout = _timeout;
        ...
        public async Task<EsxSettingsDepotContentAddOnsVersionsInfoType> GetAsync(string Name, string Version)
        {
            ...
            StringBuilder GetServiceURL = new StringBuilder("/api/esx/settings/depot-content/add-ons/{name}/versions/{version}");
            ...
            RestResponse<EsxSettingsDepotContentAddOnsVersionsInfoType> response = await restClient.ExecuteTaskAsyncWithPolicy<EsxSettingsDepotContentAddOnsVersionsInfoType>(request, cancellationToken, timeout, retry);
            ...
            else if ((int)response.StatusCode == 404) { throw new vSphereException("if OEM addon with given version is not found.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
