...
{
    public class SettingsClustersSoftwareComplianceModule
    {
        ...
        public SettingsClustersSoftwareComplianceModule(RestClient Client, CancellationToken _cancellationToken, int _timeout, int _retry)
        {
            restClient = Client;
            retry = _retry;
            timeout = _timeout;
        ...
        public async Task<EsxSettingsClusterComplianceType> GetAsync(string Cluster)
        {
            ArgumentNullException.ThrowIfNull(Cluster, "Cluster cannot be null");
            StringBuilder GetServiceURL = new StringBuilder("/api/esx/settings/clusters/{cluster}/software/compliance");
            ...
            RestResponse<EsxSettingsClusterComplianceType> response = await restClient.ExecuteTaskAsyncWithPolicy<EsxSettingsClusterComplianceType>(request, cancellationToken, timeout, retry);
            ...
            else if ((int)response.StatusCode == 404) { throw new vSphereException("If there is no cluster associated with param.name cluster in the system or if the compliance information is unavailable.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
