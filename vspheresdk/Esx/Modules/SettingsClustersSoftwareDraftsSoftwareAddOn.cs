...
{
    public class SettingsClustersSoftwareDraftsSoftwareAddOnModule
    {
        ...
        public SettingsClustersSoftwareDraftsSoftwareAddOnModule(RestClient Client, CancellationToken _cancellationToken, int _timeout, int _retry)
        {
            restClient = Client;
            retry = _retry;
            timeout = _timeout;
        ...
        public async Task SetAsync(string Cluster, string Draft, EsxSettingsAddOnSpecType RequestBody = null)
        {
            ...
            StringBuilder SetServiceURL = new StringBuilder("/api/esx/settings/clusters/{cluster}/software/drafts/{draft}/software/add-on");
        ...
        public async Task<EsxSettingsAddOnInfoType> GetAsync(string Cluster, string Draft)
        {
            ArgumentNullException.ThrowIfNull(Cluster, "Cluster cannot be null");
            ArgumentNullException.ThrowIfNull(Draft, "Draft cannot be null");
            StringBuilder GetServiceURL = new StringBuilder("/api/esx/settings/clusters/{cluster}/software/drafts/{draft}/software/add-on");
            var request = new RestRequest
            {              
                RequestFormat = DataFormat.Json,
                Method = Method.Get
            ...
            RestResponse<EsxSettingsAddOnInfoType> response = await restClient.ExecuteTaskAsyncWithPolicy<EsxSettingsAddOnInfoType>(request, cancellationToken, timeout, retry);
            ...
            else if ((int)response.StatusCode == 404) { throw new vSphereException("If there is no cluster associated with param.name cluster or no draft associated with param.name draft in the system or no addon specification is set.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else if ((int)response.StatusCode == 503) { throw new vSphereException("If the service is not available.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else if ((int)response.StatusCode == 401) { throw new vSphereException("if the caller is not authenticated.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else { throw new vSphereException(response.ErrorMessage, (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); } 
        }
        public async Task DeleteAsync(string Cluster, string Draft)
        {
            ArgumentNullException.ThrowIfNull(Cluster, "Cluster cannot be null");
            ArgumentNullException.ThrowIfNull(Draft, "Draft cannot be null");
            StringBuilder DeleteServiceURL = new StringBuilder("/api/esx/settings/clusters/{cluster}/software/drafts/{draft}/software/add-on");
            var request = new RestRequest
            {              
                RequestFormat = DataFormat.Json,
