...
namespace vspheresdk.Stats.Modules
{
    public class MetricsModule
    {
        ...
        public MetricsModule(RestClient Client, CancellationToken _cancellationToken, int _timeout, int _retry)
        {
            restClient = Client;
            retry = _retry;
            timeout = _timeout;
        ...
        public async Task<List<VstatsMetricsSummaryType>> ListAsync()
        {
            StringBuilder ListServiceURL = new StringBuilder("/api/stats/metrics");
            ...
            RestResponse<List<VstatsMetricsSummaryType>> response = await restClient.ExecuteTaskAsyncWithPolicy<List<VstatsMetricsSummaryType>>(request, cancellationToken, timeout, retry);
