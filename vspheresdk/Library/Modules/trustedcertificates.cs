...
namespace vspheresdk.Library.Modules
{
    public class TrustedCertificatesModule
    {
        ...
        public async Task<ContentTrustedCertificatesInfoType> GetAsync(string Certificate)
        {
            ...
            else if ((int)response.StatusCode == 404) { throw new vSphereException("if a trusted certificate does not exist for given id.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else { throw new vSphereException(response.ErrorMessage, (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); } 
        }
        public async Task DeleteAsync(string Certificate)
        {
            ...
            else if ((int)response.StatusCode == 404) { throw new vSphereException("if the trusted certificate specified by param.name certificate could not be found.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else if ((int)response.StatusCode == 401) { throw new vSphereException("if the user can not be authenticated.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else { throw new vSphereException(response.ErrorMessage, (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); } 
        }
    }
