...
namespace vspheresdk.Vcenter.Modules
{
    public class ClusterModule
    {
        ...
        public async Task<List<VcenterClusterSummaryType>> ListAsync(string Clusters = null, string Names = null, string Folders = null, string Datacenters = null)
        {
            ...
            else if ((int)response.StatusCode == 500) { throw new vSphereException("if more than 1000 clusters match the Cluster.FilterSpec.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
        ...
        public async Task<VcenterClusterInfoType> GetAsync(string Cluster)
        {
            ...
            if ((int)response.StatusCode == 200) { ArgumentNullException.ThrowIfNull(response.Data) ; return response.Data; }
            else if ((int)response.StatusCode == 404) { throw new vSphereException("if there is no cluster associated with cluster in the system.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else if ((int)response.StatusCode == 503) { throw new vSphereException("if the system is unable to communicate with a service to complete the request.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            ...
            else if ((int)response.StatusCode == 403) { throw new vSphereException("if the user doesnt not have the required privileges.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else { throw new vSphereException(response.ErrorMessage, (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); } 
        }
    }
}
