...
{
    public class VmGuestIdentityModule
    {
        ...
        public VmGuestIdentityModule(RestClient Client, CancellationToken _cancellationToken, int _timeout, int _retry)
        {
            restClient = Client;
            retry = _retry;
            timeout = _timeout;
        ...
        public async Task<VcenterVmGuestIdentityInfoType> GetAsync(string Vm)
        {
            ArgumentNullException.ThrowIfNull(Vm, "Vm cannot be null");
            StringBuilder GetServiceURL = new StringBuilder("/api/vcenter/vm/{vm}/guest/identity");
            ...
            RestResponse<VcenterVmGuestIdentityInfoType> response = await restClient.ExecuteTaskAsyncWithPolicy<VcenterVmGuestIdentityInfoType>(request, cancellationToken, timeout, retry);
