...
namespace vspheresdk.Vcenter.Modules
{
    public class NamespaceManagementNSXTier0GatewayModule
    {
        ...
        public async Task<List<VcenterNamespaceManagementNsxtier0GatewaySummaryType>> ListAsync(string DistributedSwitch)
        {
            ...
            else if ((int)response.StatusCode == 404) { throw new vSphereException("if no existing Tier0 Gateways associated with the given Distributed Switch can be found.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
