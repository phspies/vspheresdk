...
{
    public class VmGuestNetworkingModule
    {
        ...
        public VmGuestNetworkingModule(RestClient Client, CancellationToken _cancellationToken, int _timeout, int _retry)
        {
            restClient = Client;
            retry = _retry;
            timeout = _timeout;
        ...
        public async Task<VcenterVmGuestNetworkingInfoType> GetAsync(string Vm)
        {
            ArgumentNullException.ThrowIfNull(Vm, "Vm cannot be null");
            StringBuilder GetServiceURL = new StringBuilder("/api/vcenter/vm/{vm}/guest/networking");
            ...
            RestResponse<VcenterVmGuestNetworkingInfoType> response = await restClient.ExecuteTaskAsyncWithPolicy<VcenterVmGuestNetworkingInfoType>(request, cancellationToken, timeout, retry);
