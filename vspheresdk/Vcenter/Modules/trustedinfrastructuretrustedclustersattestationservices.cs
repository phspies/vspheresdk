...
namespace vspheresdk.Vcenter.Modules
{
    public class TrustedInfrastructureTrustedClustersAttestationServicesModule
    {
        ...
        public async Task<string> DeleteTaskAsync(string Cluster, string Service)
        {
            ...
            else if ((int)response.StatusCode == 404) { throw new vSphereException("if the Attestation Service instance or the cluster are not found.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else if ((int)response.StatusCode == 401) { throw new vSphereException("if the user can not be authenticated.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else { throw new vSphereException(response.ErrorMessage, (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); } 
            return response.Data;
        }
