...
{
    public class VmHardwareCpuModule
    {
        ...
        public VmHardwareCpuModule(RestClient Client, CancellationToken _cancellationToken, int _timeout, int _retry)
        {
            restClient = Client;
            retry = _retry;
            timeout = _timeout;
        ...
        public async Task<VcenterVmHardwareCpuInfoType> GetAsync(string Vm)
        {
            ArgumentNullException.ThrowIfNull(Vm, "Vm cannot be null");
            StringBuilder GetServiceURL = new StringBuilder("/api/vcenter/vm/{vm}/hardware/cpu");
            ...
            RestResponse<VcenterVmHardwareCpuInfoType> response = await restClient.ExecuteTaskAsyncWithPolicy<VcenterVmHardwareCpuInfoType>(request, cancellationToken, timeout, retry);
        ...
        public async Task UpdateAsync(string Vm, VcenterVmHardwareCpuUpdateType RequestBody = null)
        {
            ArgumentNullException.ThrowIfNull(Vm, "Vm cannot be null");
            StringBuilder UpdateServiceURL = new StringBuilder("/api/vcenter/vm/{vm}/hardware/cpu");
            var request = new RestRequest
            {              
                RequestFormat = DataFormat.Json,
                Method = Method.Patch
            ...
            else if ((int)response.StatusCode == 400) { throw new vSphereException("if Cpu.UpdateSpec.hotaddenabled or Cpu.UpdateSpec.hotremoveenabled is specified and the virtual machine is not powered off.if Cpu.UpdateSpec.count is specified and is greater than Cpu.Info.count Cpu.Info.hotaddenabled is false and the virtual machine is not powered off.if Cpu.UpdateSpec.count is specified and is less than Cpu.Info.count Cpu.Info.hotremoveenabled is false and the virtual machine is not powered off.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else if ((int)response.StatusCode == 503) { throw new vSphereException("if the system is unable to communicate with a service to complete the request.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else if ((int)response.StatusCode == 401) { throw new vSphereException("if the user can not be authenticated.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else if ((int)response.StatusCode == 403) { throw new vSphereException("if the user doesnt have the required privileges.", (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); }
            else { throw new vSphereException(response.ErrorMessage, (int)response.StatusCode, response.Content, response.Headers, response.ErrorException); } 
