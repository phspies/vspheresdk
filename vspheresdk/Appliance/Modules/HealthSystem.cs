...
{
    public class HealthSystemModule
    {
        ...
        public HealthSystemModule(RestClient Client, CancellationToken _cancellationToken, int _timeout, int _retry)
        {
            restClient = Client;
            retry = _retry;
            timeout = _timeout;
        ...
        {
            StringBuilder LastcheckServiceURL = new StringBuilder("/api/appliance/health/system/lastcheck");
            var request = new RestRequest
            {              
                RequestFormat = DataFormat.Json,
                Method = Method.Get
